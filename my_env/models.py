...
class Ticket(BaseModel):
    ...
    sentiment: float = Field(0.5, description="Sentiment score from 0.0 to 1.0")
    required_kb_id: Optional[str] = Field(None, description="The hidden KB ID needed for resolution")
    status: TicketStatus = Field(TicketStatus.PENDING)

class Observation(BaseModel):
    active_ticket: Optional[Ticket] = Field(None, description="Current ticket details")
    queue_backlog: int = Field(..., description="Tickets remaining in queue")
    agent_fatigue: float = Field(..., description="Current fatigue (0.0 to 1.0)")
    available_kb_articles: Dict[str, str] = Field(..., description="KB ID mapping to article titles")
...
class Action(BaseModel):
    ...
    reasoning: str = Field(..., description="Internal thought process for the action")
