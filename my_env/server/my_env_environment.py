...
class SupportTicketEnv:
    """
    Core MDP Logic for the Support Ticket Environment.
    Handles transitions, reward calculation, and state management.
    """
    ...
    def _get_obs(self) -> Observation:
        ...
        return Observation(
            ...
            available_kb_articles=self.kb_data
        )

    def step(self, action: Action) -> Tuple[Observation, float, bool, Dict[str, Any]]:
        if self.state.current_index >= len(self.state.all_tickets):
            return self._get_obs(), 0.0, True, {"msg": "Queue empty"}

        ticket = self.state.all_tickets[self.state.current_index]
        self.state.steps_taken += 1
        reward = -0.5  # Base Step Penalty (Time Pressure)
        info = {}
